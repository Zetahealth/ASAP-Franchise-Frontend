...
export default function ResetPassword() {
  ...
  // ✅ Detect if it's for admin or user based on current path
  const isAdmin = location.pathname.includes("/admin");

  useEffect(() => {
    if (!email) {
      navigate(isAdmin ? "/admin/forgot-password" : "/user/forgot-password");
    }
  }, [email, navigate, isAdmin]);
      ...
      <span className="relative flex items-center">
        <span
          className={`transition-all duration-300 transform ${
            isMet ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-2"
          }`}
        >
          <Check size={16} />
        </span>
        ...
        <form onSubmit={handleReset} className="space-y-5">
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`w-full border rounded px-3 py-2 pr-10 focus:outline-none focus:ring-2 focus:ring-[#8b2f2f] ${
            ...
            {confirmPwd && confirmPwd === newPwd && (
              <span className="absolute right-3 top-9 text-green-600 transition-all duration-300 ease-in-out">
                <Check size={18} />
              </span>
            )}
            {confirmPwd && confirmPwd !== newPwd && (
